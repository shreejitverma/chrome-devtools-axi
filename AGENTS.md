...
The vendored `.agents/skills/no-mistakes/SKILL.md` is internal agent guidance and must keep `metadata.internal: true` in frontmatter so `npx skills add ... --list --full-depth` discovers only the public chrome-devtools-axi skill.
...
- Keep `.agents/skills/no-mistakes/SKILL.md` marked with `metadata.internal: true`; it is vendored for local agents and should not appear in public skill discovery.
